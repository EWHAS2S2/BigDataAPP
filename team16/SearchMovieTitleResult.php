						...
						<div class="inner">
							...
							<!-- Content -->
								<section>
                                    ...
                                    <form>
                                        <h2>Searching Movie Title Result</h2> 
                                    </form>
                                    ...
                                    <?php
                                        ...
                                        //print($_POST['searchType']);

                                        $sql = "SELECT movie_id, title FROM movie_runtime WHERE title LIKE '%".$_POST['searchword']."%'"; // 쿼리
                                        $result = mysqli_query($connect, $sql); // 쿼리 실행
                                                
                                            while ($row = mysqli_fetch_assoc($result)) { // 집합에서 하나씩 레코드를 꺼내 연관배열로 저장
                                                echo $row['title']."<br>"; // 연관배열 인덱스 접근
                                            }
